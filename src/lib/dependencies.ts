...
export function resolveDependencyString (path: string, dependency: string, options: Options, parent?: DependencyTree): Promise<DependencyTree> {
  return new Promise(resolve => {
    return resolve(resolveDependency(path, parseDependency(dependency), options, parent))
  })
}
    ...
    return arrify(dependencies[name])
      .reduce(function (result: Promise<DependencyTree>, dependency: string) {
        return result.then(function (tree) {
          ...
          if (tree.missing) {
            return resolveDependencyString(path, dependency, extend(options, { dev: false }), parent)
          }
